...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder, load_img=False, load_sem=True):
    cam_infos = []
    for idx, key in enumerate(cam_extrinsics):
        ...
        if load_sem:
            if vlm == 'clip':
                ...
                raise NotImplementedError(f"VLM {vlm} not implemented")
...
def readColmapSceneInfo(path, images, eval, llffhold=8, load_img=False, load_sem=True):
    try:
    ...
    cam_infos_unsorted = readColmapCameras(cam_extrinsics=cam_extrinsics, cam_intrinsics=cam_intrinsics, images_folder=os.path.join(path, reading_dir), load_img=load_img, load_sem=load_sem)
...
def readNerfSyntheticInfo(path, white_background, eval, extension=".png", load_img=True, load_sem=True):
